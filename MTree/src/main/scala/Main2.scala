...
object Main2 extends App{
    val writer = new PrintWriter(new File("benchmark3D.txt"));
    for (k <- Seq(10,50,100)) {
        var mt = new MTree[String](M = k);

        var bufferedSource = io.Source.fromFile("points3D_small.dat");
        var i = 1;
        var duration = 0.0;
        var v = Vector[Entry[String]]();
        var bPoints = Vector[Point3D]();
        for (line <- bufferedSource.getLines) {
            val cols = line.split(",");

            val pt = new Point3D(cols(1).toDouble, cols(2).toDouble, cols(3).toDouble);
    ...
    for (k <- Seq(10,50,100)) {
        ...
        var bufferedSource = io.Source.fromFile("points3D_medium.dat");
        var i = 1;
        var duration = 0.0;
        var v = Vector[Entry[String]]();
        var bPoints = Vector[Point3D]();
    ...
    for (k <- Seq(10,50,100)) {
        ...
        var bufferedSource = io.Source.fromFile("points3D_large.dat");
        var i = 1;
        var duration = 0.0;
        var v = Vector[Entry[String]]();
        var bPoints = Vector[Point3D]();
